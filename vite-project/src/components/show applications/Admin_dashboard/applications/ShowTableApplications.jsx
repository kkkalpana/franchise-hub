...
function ShowTableApplications({ jsonObj, onAccept, onReject, onFranchise }) {
  return (
    <div className="min-h-screen h-full bg-white flex  py-6 gap-2">
      <div className="w-full">
        {/* <div className="flex justify-end bg-[#222E3A]/[6%] py-2 px-2  border-t border-x border-black mt-2">
          ...
          <table className="w-full">
            <thead className="rounded-lg  text-white font-medium w-full">
              <tr className="bg-slate-400 text-lg">
                ...
                <th
                  className={`py-5 px-7 text-[#212B36] text-center whitespace-nowrap  `}
                >
                  FIRST NAME
                </th>
                <th className="py-5 px-7 text-[#212B36] text-center gap-1 text-[#212B36] whitespace-nowrap">
                  LAST NAME
                </th>
                ...
                {/* <th className={`py-4 px-7 text-[#212B36] font-bold  `}>
                    STATUS
                  </th> */}
            ...
            <tbody>
              {jsonObj?.map((obj, index) => (
                ...
                >
                  ...
                  {/*-------------------- Status --------------------*/}
                  {/* <td className={`py-5 px-7  font-bold`}>
                      {obj.status == 0 ? (
                        <span className=""> {status_array[0]}</span>
                      ) : obj.status == 1 ? (
                        ...
                        <span className="text-blue-600">{status_array[2]}</span>
                      ) : (
                        <span className="text-red-600">{status_array[3]}</span>
                      )}
                    </td> */}

                  {/*-------------------- Actions --------------------*/}
                  <td className={`py-5 px-7 font-normal `}>
                    <div className="flex items-center justify-center gap-3">
                      ...
                      {obj.status == 0 ? (
                        <button
                          ...
                          className="flex-1 py-3 rounded-lg bg-green-600 hover:bg-green-700 px-6 font-medium leading-tight text-white shadow-md transition duration-150 ease-in-out hover:shadow-lg focus:shadow-lg focus:outline-none active:shadow-lg"
                        >
                          Accept
                        </button>
                      ) : null}
                      ...
                      {obj.status == 0 || obj.status == 1 ? (
                        <button
                          ...
                          className="flex-1 py-3 rounded-lg bg-red-600 hover:bg-red-700 px-6 font-medium  leading-tight text-white shadow-md transition  duration-150 ease-in-out hover:shadow-lg focus:shadow-lg focus:outline-none active:shadow-lg"
                        >
                          Reject
                        </button>
                      ) : null}
                      ...
                      {/* ---------------- Status = 2 --------------------*/}
                      {obj.status == 2 ? (
                        <button className="flex-1 py-3 disabled rounded-lg bg-blue-400 px-6 font-medium  leading-tight cursor-default text-white shadow-md">
                          Franchise Granted
                        </button>
                      ) : null}
                      ...
                      {/*---------------- Status = 3 --------------------*/}
                      {obj.status == 3 ? (
                        <button className="flex-1 py-3 disabled rounded-lg bg-gray-500 px-6 font-medium  leading-tight cursor-default text-white shadow-md">
                          Applicant Rejected
                        </button>
                      ) : null}
                    </div>
...
export default ShowTableApplications;
